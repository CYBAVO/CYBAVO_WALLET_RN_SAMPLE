...
class TabBar extends Component<Props, State> {
  ...
  componentDidMount() {
    this._animateListenerId = this.props.scrollValue.addListener(
  ...
  shouldComponentUpdate(nextProps: Props, nextState: State) {
    ...
    const serializedNextState =
      JSON.stringify(nextProps) + JSON.stringify(nextState);
  ...
  handleScrolling = (event: { value: number }) => {
    const { value } = event;
    const dx =
      (this.offsetCollection && this.offsetCollection._interpolation(value)) ||
      0;
    const scaleX =
    ...
    if (this.scrollOffsetsCollection) {
      const scrollOffset =
        this.scrollOffsetsCollection &&
        this.scrollOffsetsCollection._interpolation(value);
      if (this.scrollView) {
  ...
  calculateInterpolations = () => {
    ...
    for (let i = 1, len = inputRange.length; i < len; i += 1) {
      ...
      if (offset + tabWidth + nextTabWidth + 2 * marginValue >= scrollWidth) {
        if (isLast) {
          scrollOffset =
            offset - (tabContainerWidth - (tabWidth + marginValue));
        } else {
          ...
          scrollOffset =
            offset -
            (tabContainerWidth - (tabWidth + nextTabWidth + 1.3 * marginValue));
        }
      } else {
        scrollOffset =
          offset -
  ...
  renderUnderline() {
    ...
    return (
      <Animated.View ref={this.setUnderlineRef} style={[tabUnderlineStyle]} />
    );
  }
  ...
  renderTab = (
    ...
    return (
      <TouchableOpacity
        ...
        {badge != null && parseInt(badge, 10) > 0 && (
          <View
            style={[
              styles.badgeBubble,
      ...
      <View
        ...
        <ScrollView
          ...
          {tabs.map((tab: TabType, page: number) => {
            ...
            const onTabLayout = (event: Object) =>
              this.onTabLayout(event, page);
            return renderTab(
              tab,
              page,
              isTabActive,
