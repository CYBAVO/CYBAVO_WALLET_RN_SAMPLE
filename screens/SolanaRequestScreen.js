...
import {
  approveRequest,
  approveSession,
  approveSessionRequest, dequeueRequest,
  fetchApiHistory,
  killAllSession,
  rejectRequest,
...
const SolanaRequestScreen: () => React$Node = ({ theme }) => {
  const refs = [useRef()];
  ...
  const isTransactionRequest =
    payload.method === SOLANA_SIGNING_METHODS.SOLANA_SIGN_TRANSACTION;
  ...
  const _walletConnectSignMessage = async (
    ...
    try {
      ...
      let b58 = result.signedMessage;
      await dispatch(approveSessionRequest(requestEvent, { signature: b58 }));
      _leaveAndDequeue();
      dispatch(fetchApiHistory());
    } catch (error) {
  ...
  const _walletConnectSignTransaction = async (
    ...
    try {
      ...
      switch (type) {
        case AUTH_TYPE_SMS:
          tag = tag + 'Sms';
          result = await Wallets.walletConnectSignTransactionCommonSms(
            actionToken,
            code,
            wallet.walletId,
            tx,
        ...
        case AUTH_TYPE_BIO:
          ...
          result = await Wallets.walletConnectSignTransactionCommonBio(
            I18n.t('bio_msg'),
            I18n.t('cancel'),
            wallet.walletId,
            tx,
          ...
          break;
        case AUTH_TYPE_OLD:
          result = await Wallets.walletConnectSignTransactionCommon(
            wallet.walletId,
            tx,
            pinSecret
          );
      ...
      console.debug(tag + result.signedTx);
      FileLogger.debug(
        `${tag} success: ${result.signedTx}, walletId:${wallet.walletId}`
      );
      await dispatch(
        approveSessionRequest(requestEvent, {
  ...
  const _getSubViewStep0 = () => {
    switch (payload.method) {
      case SOLANA_SIGNING_METHODS.SOLANA_SIGN_MESSAGE:
        return _getSignView(payload.params.pubkey, payload.params.message);
      case SOLANA_SIGNING_METHODS.SOLANA_SIGN_TRANSACTION:
      default:
        return _getSignTypedDataView(payload.params);
    }
  };
        ...
        <View style={{ marginTop: 8 }}>
          ...
          <RoundButton2
            ...
            onPress={async () => {
              ...
              if (
                payload.method === SOLANA_SIGNING_METHODS.SOLANA_SIGN_MESSAGE
              ) {
                setResult({
                  type: TYPE_CONFIRM,
...
export default withTheme(SolanaRequestScreen);
