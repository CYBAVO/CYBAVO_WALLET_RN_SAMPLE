...
public class LineLoginManager extends ReactContextBaseJavaModule {
    ...
    public void login(final Promise promise) {
        try {
            ...
            LineAuthenticationParams params = new LineAuthenticationParams.Builder().scopes(Arrays.asList(Scope.PROFILE, Scope.OPENID_CONNECT)).build();
            Intent intent = LineLoginApi.getLoginIntent(context, channelId, params);
    ...
    @ReactMethod
    public void loginWithPermissions(final ReadableArray scopes, final Promise promise) {
        try {
            currentPromise = promise;
            Context context = getCurrentActivity().getApplicationContext();
            String channelId = context.getString(R.string.line_channel_id);
